...
(() => {
 ...
 function history(){
   ...
   for(const n of model.history()){
     if(n.end)continue;const row=document.createElement('article');row.className='log-entry';row.dataset.voice=voiceKey(n.speaker||n.phoneTitle);if(n.speaker){const who=document.createElement('small');who.textContent=n.speaker;row.append(who);}const unreadLines=n.phone?.slice(n.noticeBefore?story.nodes[n.noticeBefore].phone?.length||0:0);if(unreadLines?.length){const label=document.createElement('small');label.textContent='与'+(n.phoneTitle||'陈屿')+'的消息';row.append(label);for(const [who,words] of unreadLines)paragraph(who+'：'+words,row,'log-message');}if(n.phoneStatus&&!n.noticeBefore)paragraph((n.phoneTitle||'陈屿')+' · '+n.phoneStatus,row,'log-message');paragraph(n.text,row,'');body.append(row);
   }
