...
module.exports=async function(win){
 const output=path.resolve(process.cwd(),'work/qa-v07');fs.mkdirSync(output,{recursive:true});
 ...
 await js(`localStorage.setItem('huicheng.chapters.v07.preferences',JSON.stringify({speed:0,size:30,volume:30,delay:3,mute:false,typeScale:3,staging:false}));`);
 ...
 const interaction=await js(`(()=>{const $=id=>document.getElementById(id),snap=()=>JSON.parse(localStorage.getItem('huicheng.chapters.v07.auto'));const issues=[];
 ...
 const stageState=()=>js(`({id:JSON.parse(localStorage.getItem('huicheng.chapters.v07.auto')).path.at(-1),hidden:document.getElementById('phone').hidden,busy:document.getElementById('dialogue').getAttribute('aria-busy'),phase:document.getElementById('app').dataset.stage})`);
