...
module.exports=async function(win){
 const output=path.resolve(process.cwd(),'work/qa-v11');fs.mkdirSync(output,{recursive:true});
 ...
 await js(`localStorage.setItem('huicheng.chapters.v11.preferences',JSON.stringify({speed:0,size:30,volume:30,delay:3,mute:false,typeScale:3,staging:false}));`);
 ...
 assert.equal(await js(`JSON.parse(localStorage.getItem('huicheng.chapters.v11.auto')).path.at(-1)`),'c5-001');
 await js(`localStorage.removeItem('huicheng.chapters.v11.auto')`);
 ...
 const knockEvents=await js('qaKnockEvents');assert.equal(knockEvents.filter(e=>e.type==='start').length,Object.values(require('./story.js').nodes).reduce((sum,n)=>sum+(require('./knock.js').cues[n.sound]?.count||0),0));assert.ok(knockEvents.filter(e=>e.type==='start').every(e=>e.length>30000));
 ...
 await js(`window.qaGoto=(id)=>{const path=[];let n=HCStory.nodes[HCStory.start];while(n){path.push(n.id);if(n.id===id)break;n=HCStory.nodes[n.next];}localStorage.setItem('huicheng.chapters.v11.auto',JSON.stringify({story:HCStory.id,version:HCStory.version,path,savedAt:Date.now()}));document.getElementById('end-title').click();document.getElementById('continue').click();};void 0;`);
 ...
 assert.equal(await js(`JSON.parse(localStorage.getItem('huicheng.chapters.v11.auto')).path.at(-1)`),evidence);
 ...
 const records=await js(`Object.values(HCStory.nodes).filter(n=>n.evidence&&!n.noticeFor).map(n=>({id:n.id,src:n.evidence.src}))`);
 for(const record of records){
  await js(`qaGoto(${JSON.stringify(record.id)});document.querySelector('.evidence-button').click();`);
  assert.equal(await js(`document.querySelector('.evidence-image').decode().then(()=>document.querySelector('.evidence-image').naturalWidth>0)`),true);
  await capture('record-'+record.id);await js(`document.getElementById('close-panel').click()`);
 }
 for(const chapter of [3,4]){
  const id=await js(`Object.values(HCStory.nodes).find(n=>n.id.startsWith('c${chapter+1}-')&&n.noticeFor&&n.phone?.some(p=>p[1].includes(${JSON.stringify(chapter===3?'别试钥匙':'拿到了吗')}))).id`);
  await js(`qaGoto(${JSON.stringify(id)})`);await capture('chapter'+(chapter+1)+'-pressure');
 }
 ...
 const interaction=await js(`(()=>{const $=id=>document.getElementById(id),snap=()=>JSON.parse(localStorage.getItem('huicheng.chapters.v11.auto'));const issues=[];
 ...
 const reaction=await js(`(()=>{const m=HCModel.createModel(HCStory),found=[];while(!m.node().end){if(m.presentation().heldPhone&&!m.node().phone&&m.node().id.startsWith('c1-'))found.push(m.node());m.next();}return found.sort((a,b)=>b.text.length-a.text.length)[0].id;})()`);
 ...
 const finalPhone=await js(`Object.values(HCStory.nodes).find(n=>n.phone?.some(p=>p[1]==='我去过你那里。里面有人。')).id`);await js(`qaGoto(${JSON.stringify(finalPhone)})`);await pause(1000);await capture('compact-phone');
 ...
 const stageState=()=>js(`({id:JSON.parse(localStorage.getItem('huicheng.chapters.v11.auto')).path.at(-1),hidden:document.getElementById('phone').hidden,busy:document.getElementById('dialogue').getAttribute('aria-busy'),phase:document.getElementById('app').dataset.stage})`);
